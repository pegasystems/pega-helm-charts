...
{{- define "pegaDeployConfig" }}pega-deploy-config{{- end }}
...
{{- define "performOnlyDeployment" }}
  {{- if (eq .Values.actions.execute "deploy") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
{{- define "performInstall" }}
  {{- if or (eq .Values.actions.execute "install") (eq .Values.actions.execute "install-deploy") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
{{- define "performUpgrade" }}
  {{- if or (eq .Values.actions.execute "upgrade") (eq .Values.actions.execute "upgrade-deploy") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
{{- define "performOnlyUpgrade" }}
  {{- if (eq .Values.actions.execute "upgrade") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
{{- define "performDeployment" }}
  {{- if or (eq .Values.actions.execute "deploy") (eq .Values.actions.execute "install-deploy") (eq .Values.actions.execute "upgrade-deploy") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
{{- define "performInstallAndDeployment" }}
  {{- if (eq .Values.actions.execute "install-deploy") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
{{- define "performUpgradeAndDeployment" }}
  {{- if (eq .Values.actions.execute "upgrade-deploy") -}}
    true
  {{- else -}}
    false
  {{- end -}}
...
- name: wait-for-rolling-updates
  image: dcasavant/k8s-wait-for
  command: ['sh', '-c', 'kubectl rollout status deployment/{{ include "pegaWebName" .}} --namespace {{ .Release.Namespace }} && kubectl rollout status deployment/{{ include "pegaBatchName" .}} --namespace {{ .Release.Namespace }} && kubectl rollout status statefulset/{{ include "pegaStreamName" .}} --namespace {{ .Release.Namespace }}']
{{- include "initContainerEnvs" .}}
{{- end }}
...
{{- if or (eq .Values.provider "aks") (eq .Values.provider "pks") -}}
{{ $apiserver := index .Values.installer.upgrade "kube-apiserver" }}
  env:
