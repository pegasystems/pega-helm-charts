...
func TestInstallDeployActionShouldNotRenderDeployments(t *testing.T) {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			"actions.execute": "deploy",
		},
	}

	// with action as 'deploy' below templates should not be rendered
	output := helm.RenderTemplate(t, options, helmChartPath, []string{
		...
		"templates/pega-install-environment-config.yaml",
		"templates/pega-installer-config.yaml",
	})
	...
	require.Equal(t, batchDeploymemtObj.Spec.Template.Labels["app"], "pega-batch")
	require.NotEmpty(t, batchDeploymemtObj.Spec.Template.Annotations["config-check"])
	...
	require.Equal(t, batchDeploymemtSpec.InitContainers[0].Name, "wait-for-pegasearch")
	require.Equal(t, batchDeploymemtSpec.InitContainers[0].Image, "busybox:1.27.2")
	require.Equal(t, batchDeploymemtSpec.InitContainers[0].Command, []string{"sh", "-c", "until $(wget -q -S --spider --timeout=2 -O /dev/null http://pega-search); do echo Waiting for search to become live...; sleep 10; done;"})
	require.Equal(t, batchDeploymemtSpec.InitContainers[1].Name, "wait-for-cassandra")
	require.Equal(t, batchDeploymemtSpec.InitContainers[1].Image, "cassandra:3.11.3")
	require.Equal(t, batchDeploymemtSpec.InitContainers[1].Command, []string{"sh", "-c", "until cqlsh -u \"dnode_ext\" -p \"dnode_ext\" -e \"describe cluster\" release-name-cassandra 9042 ; do echo Waiting for cassandra to become live...; sleep 10; done;"})
	...
	fmt.Println("efrerwet")
	fmt.Printf("%v\n",batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("cpu")].OpenAPISchemaFormat())
	fmt.Printf("%v\n",batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("memory")].OpenAPISchemaFormat())
	
	//fmt.Printf("%v\n",batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("cpu")].i.value)
	//fmt.Printf("%v\n",batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("memory")].i.value)
	
	//fmt.Printf("%v\n",batchDeploymemtSpec.Containers[0].Resources.Limits)
	//fmt.Printf("%v\n",batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("memory")])
	//degah
	//require.Equal(t, batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("cpu")].Format, k8sresource.Quantity)
	//require.Equal(t, batchDeploymemtSpec.Containers[0].Resources.Limits[k8score.ResourceName("memory")], "8Gi")
    //require.Equal(t, batchDeploymemtSpec.Containers[0].Resources.Requests[k8score.ResourceName("cpu")].Format, k8sresource.Quantity)
	...
	require.Equal(t, streamDeploymentObj.Spec.Template.Labels["app"], "pega-stream")
	require.NotEmpty(t, streamDeploymentObj.Spec.Template.Annotations["config-check"])
	...
	require.Equal(t, streamDeploymentSpec.InitContainers[0].Name, "wait-for-pegasearch")
	require.Equal(t, streamDeploymentSpec.InitContainers[0].Image, "busybox:1.27.2")
	require.Equal(t, streamDeploymentSpec.InitContainers[0].Command, []string{"sh", "-c", "until $(wget -q -S --spider --timeout=2 -O /dev/null http://pega-search); do echo Waiting for search to become live...; sleep 10; done;"})
	...
	require.Equal(t, webDeploymentSpec.InitContainers[0].Name, "wait-for-pegasearch")
	require.Equal(t, webDeploymentSpec.InitContainers[0].Image, "busybox:1.27.2")
	require.Equal(t, webDeploymentSpec.InitContainers[0].Command, []string{"sh", "-c", "until $(wget -q -S --spider --timeout=2 -O /dev/null http://pega-search); do echo Waiting for search to become live...; sleep 10; done;"})
