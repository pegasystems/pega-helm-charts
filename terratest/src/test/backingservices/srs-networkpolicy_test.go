...
func TestSRSServiceNetworkPolicy(t *testing.T){
	...
	VerifySRSServiceNetworkPolicy(t, &networkPolicyObj)
}

func VerifySRSServiceNetworkPolicy(t *testing.T, networkPolicySvcObj *networkingv1.NetworkPolicy) {
	require.Equal(t, networkPolicySvcObj.Spec.PodSelector.MatchLabels["app.kubernetes.io/name"], "srs-service")
	require.Equal(t, networkPolicySvcObj.Spec.PodSelector.MatchLabels["networking/allow-internet-egress"], "true")
	require.ElementsMatch(t, networkPolicySvcObj.Spec.PolicyTypes, []networkingv1.PolicyType{ "Ingress","Egress"})
	// ingress
