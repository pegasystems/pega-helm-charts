...
func VerifyPegaHPAs(t *testing.T, yamlContent string, options *helm.Options) {
	...
	for index, hpaInfo := range hpaSlice {
		if index >= 0 && index <= 1 {
			...
			if index == 0 {
				VerifyPegaHpa(t, &pegaHpaObj, hpa{getObjName(options, "-web-hpa"), getObjName(options, "-web"), "Deployment", "apps/v1"}, true, true)
			} else {
				VerifyPegaHpa(t, &pegaHpaObj, hpa{getObjName(options, "-batch-hpa"), getObjName(options, "-batch"), "Deployment", "apps/v1"}, true, false )
			}
		}
	}
}
