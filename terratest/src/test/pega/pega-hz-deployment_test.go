...
func VerifyHazelcastDeployment(t *testing.T, yamlContent string) {
	...
	for index, statefulInfo := range statefulSlice {
		if index >= 1 {
			...
			require.Equal(t, statefulsetSpec.Volumes[2].Name, "hazelcast-volume-credentials")
			...
			require.Equal(t, statefulsetSpec.Containers[0].VolumeMounts[2].Name, "hazelcast-volume-credentials")
			require.Equal(t, statefulsetSpec.Containers[0].VolumeMounts[2].MountPath, "/opt/hazelcast/secrets")
		}
	}
}
