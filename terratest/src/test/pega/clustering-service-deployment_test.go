...
func TestClusteringServiceDeployment(t *testing.T) {
	...
	for _, vendor := range supportedVendors {
		...
		for _, operation := range supportedOperations {
			...
			VerifyClusteringServiceDeployment(t, yamlContent, false)

		}
	}
}

func TestClusteringServiceDeploymentWithSSL(t *testing.T) {
	var supportedVendors = []string{"k8s", "openshift", "eks", "gke", "aks", "pks"}
	var supportedOperations = []string{"deploy", "install-deploy"}
	...
	for _, vendor := range supportedVendors {
		...
		for _, operation := range supportedOperations {
			...
			yamlContent := RenderTemplate(t, options, helmChartPath, []string{"charts/hazelcast/templates/clustering-service-deployment.yaml"})
			VerifyClusteringServiceDeployment(t, yamlContent, true)

		}
	}
}

func VerifyClusteringServiceDeployment(t *testing.T, yamlContent string, ssl bool) {
