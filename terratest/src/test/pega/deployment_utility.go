...
// VerifyPegaIngressWithcosmosUIEnabled - Performs Pega Ingress assertions with the values as provided in default values.yaml when the cosmosUI service is enabled.
...
// VerifycosmosUIService - Verifies cosmosUI service deployment used by cosmosUI pod with the values as provided in default values.yaml
func VerifyConstellationService(t *testing.T, helmChartPath string, options *helm.Options) {
	...
	require.Equal(t, constellationServiceObj.Spec.Selector["app"], "cosmosUI")
