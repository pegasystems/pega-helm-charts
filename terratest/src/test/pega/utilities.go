...
func VerifyInitContinerData(t *testing.T, containers []k8score.Container, options *helm.Options) {
	...
	for i := 0; i < len(containers); i++ {
		...
		if name == "wait-for-pegainstall" {
			...
			require.Equal(t, "busybox:1.31.0", container.Image)
			require.Equal(t, []string{"sh", "-c", "set -e counter=0; while [ $(wget -q -O - \"http://hazelcast-service.default:5701/hazelcast/health/cluster-size\" /dev/null) -ne 3 ] || [ $(wget -S \"http://hazelcast-service.default:5701/hazelcast/health/cluster-safe\" 2>&1 | grep \"HTTP/\" | awk '{print $2}') -ne 200 ]; do echo \"waiting for hazelcast pods to start and join the cluster...\" ; counter=$(($counter+5)); sleep 5; if [ $counter -gt 150 ]; then echo \"Timeout Reached. Hazelcast pods failed to join the cluster.\"; exit 1; fi done; echo \"Hazelcast cluster is up now\"; exit 0;\n"}, container.Args)
		} else {
