...
func assertCredentialVolumeAndMount(t *testing.T, tierYaml string, options *helm.Options, shouldHaveVol bool) {
	...
	for _, vol := range pod.Volumes {
		if vol.Name == "pega-volume-credentials" {
			...
			require.Equal(t, len(sources), 7)
			require.Equal(t, sources[0].Secret.LocalObjectReference.Name, "pega-credentials-secret")
			require.Equal(t, sources[1].Secret.LocalObjectReference.Name, "db-secret")
			require.Equal(t, sources[2].Secret.LocalObjectReference.Name, "dds-secret")
			require.Equal(t, sources[3].Secret.LocalObjectReference.Name, "kafka-secret")
			require.Equal(t, sources[4].Secret.LocalObjectReference.Name, "pega-diagnostic-secret")
			require.Equal(t, sources[5].Secret.LocalObjectReference.Name, "customArtifactory-secret")
			require.Equal(t, sources[6].Secret.LocalObjectReference.Name, "hazelcast-secret")
			foundVol = true
			break
		}
	}
