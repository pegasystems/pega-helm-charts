...
func VerifyPegaDeployment(t *testing.T, deploymentObj *appsv1.Deployment, expectedDeployment pegaDeployment, options *helm.Options) {
	...
	require.Equal(t, intstr.FromString("25%"), *deploymentObj.Spec.Strategy.RollingUpdate.MaxSurge)
	require.Equal(t, intstr.FromString("25%"), *deploymentObj.Spec.Strategy.RollingUpdate.MaxUnavailable)
...
func VerifyDeployment(t *testing.T, pod *k8score.PodSpec, expectedSpec pegaDeployment, options *helm.Options) {
	...
	envIndex++
	require.Equal(t, "MAX_METASPACE", pod.Containers[0].Env[envIndex].Name)
	require.Equal(t, "1024m", pod.Containers[0].Env[envIndex].Value)
	...
	require.Equal(t, "10Gi", pod.Containers[0].Resources.Limits.Memory().String())
	require.Equal(t, "2", pod.Containers[0].Resources.Requests.Cpu().String())
	require.Equal(t, "10Gi", pod.Containers[0].Resources.Requests.Memory().String())
	...
	require.Equal(t, int32(300), pod.Containers[0].LivenessProbe.InitialDelaySeconds)
	...
	require.Equal(t, int32(300), pod.Containers[0].ReadinessProbe.InitialDelaySeconds)
	require.Equal(t, int32(20), pod.Containers[0].ReadinessProbe.TimeoutSeconds)
	require.Equal(t, int32(30), pod.Containers[0].ReadinessProbe.PeriodSeconds)
