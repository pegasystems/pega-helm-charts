...
func VerifyPegaDeployment(t *testing.T, deploymentObj *appsv1.Deployment, expectedDeployment pegaDeployment, options *helm.Options) {
	...
	require.Equal(t, *deploymentObj.Spec.Strategy.RollingUpdate.MaxSurge, int32(1))
	require.Equal(t, *deploymentObj.Spec.Strategy.RollingUpdate.MaxUnavailable, int32(0))
