...
func VerifyPegaDeployment(t *testing.T, deploymentObj *appsv1.Deployment, expectedDeployment pegaDeployment, options *helm.Options) {
	...
	require.Equal(t, *deploymentObj.Spec.Strategy.RollingUpdate.MaxSurge, intstr.FromString("1"))
	require.Equal(t, *deploymentObj.Spec.Strategy.RollingUpdate.MaxUnavailable, intstr.FromString("0"))
...
func VerifyDeployment(t *testing.T, pod *k8score.PodSpec, expectedSpec pegaDeployment, options *helm.Options) {
	...
	require.Equal(t, pod.Containers[0].LivenessProbe.InitialDelaySeconds, int32(0))
	...
	require.Equal(t, pod.Containers[0].ReadinessProbe.InitialDelaySeconds, int32(0))
