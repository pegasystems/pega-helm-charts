...
/* func TestUpgradeDeployActionStandardDeployment(t *testing.T) {
	t.Parallel()
	//t.Skip()
	// Path to the helm chart we will test
...
func validateUpgradeJobs(t *testing.T) {
	...
	for index, installerInfo := range installerSlice {
			...
			VerifyJob(t, options, &installerJobObj, pegaJob{"pega-pre-upgrade", []string{}, "pega-upgrade-environment-config"})
