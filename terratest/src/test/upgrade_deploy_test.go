 ...
 */
	...
	require.Equal(t, string(secretData["PEGA_DIAGNOSTIC_PASSWORD"]), "")
 	...
 	// pega-upgrade-environment-config.yaml
...
/*     installerConfig := helm.RenderTemplate(t, options, helmChartPath, []string{"templates/pega-installer-config.yaml"})
	...
	splitArray := array.Split(installerJob, -1)

	println(len(splitArray))

	//fmt.Printf("%q\n", a.Split(installerJob, -1))
	for index,data := range splitArray {
		if index == 1 {
			preUpgradeValidation(index,data)
		}
		if index == 2 {
			upgradeValidation(index,data)
		}
		if index == 3 {
			postUpgradeValidation(index,data)
		}
	}
...
func preUpgradeValidation(index int, data string) {	
	println("-------------------------------preUpgradeValidation---------------------------------------------------------")
	println(index)
	println(data)

}
func upgradeValidation(index int, data string) {	
	println("-------------------------------upgradeValidation---------------------------------------------------------")
	println(index)
	println(data)
}
func postUpgradeValidation(index int, data string) {
	println("-----------------------------postUpgradeValidation-----------------------------------------------------------")	
	println(index)
	println(data)
