...
func TestUpgradeActionShouldNotRenderDeployments(t *testing.T) {
...
func TestUpgradeActionInstallJob(t *testing.T) {
	...
	*/
	...
	upgradeJob := helm.RenderTemplate(t, options, helmChartPath, []string{"charts/installer/templates/pega-installer-job.yaml"})
	var upgradeJobObj k8sbatch.Job
	helm.UnmarshalK8SYaml(t, upgradeJob, &upgradeJobObj)
	upgradeJobSpec := upgradeJobObj.Spec.Template.Spec
	upgradeJobConatiners := upgradeJobObj.Spec.Template.Spec.Containers

	var containerPort int32 = 8080
	var volumeDefaultMode int32 = 420
	var volumeDefaultModePtr = &volumeDefaultMode

	require.Equal(t, upgradeJobSpec.Volumes[0].Name, "pega-volume-credentials")
	require.Equal(t, upgradeJobSpec.Volumes[0].VolumeSource.Secret.SecretName, "pega-credentials-secret")
	require.Equal(t, upgradeJobSpec.Volumes[0].VolumeSource.Secret.DefaultMode, volumeDefaultModePtr)
	require.Equal(t, upgradeJobSpec.Volumes[1].Name, "pega-volume-installer")
	require.Equal(t, upgradeJobSpec.Volumes[1].VolumeSource.ConfigMap.LocalObjectReference.Name, "pega-installer-config")
	require.Equal(t, upgradeJobSpec.Volumes[1].VolumeSource.ConfigMap.DefaultMode, volumeDefaultModePtr)

	require.Equal(t, upgradeJobConatiners[0].Name, "pega-db-upgrade")
	require.Equal(t, upgradeJobConatiners[0].Image, "YOUR_INSTALLER_IMAGE:TAG")
	require.Equal(t, upgradeJobConatiners[0].Ports[0].ContainerPort, containerPort)
	require.Equal(t, upgradeJobConatiners[0].VolumeMounts[0].Name, "pega-volume-installer")
	require.Equal(t, upgradeJobConatiners[0].VolumeMounts[0].MountPath, "/opt/pega/config")
	require.Equal(t, upgradeJobConatiners[0].VolumeMounts[1].Name, "pega-volume-credentials")
	require.Equal(t, upgradeJobConatiners[0].VolumeMounts[1].MountPath, "/opt/pega/secrets")
	require.Equal(t, upgradeJobConatiners[0].EnvFrom[0].ConfigMapRef.LocalObjectReference.Name, "pega-upgrade-environment-config")
	
	require.Equal(t, upgradeJobSpec.ImagePullSecrets[0].Name, "pega-registry-secret")
	
	require.Equal(t, upgradeJobSpec.RestartPolicy, k8score.RestartPolicy("Never"))	 
}

func TestUpgradeActionInstallerEnvironmentConfig(t *testing.T) {
...
func TestUpgradeActionRegistrySecret(t *testing.T){
...
func TestUpgradeActionCredentailsSecret(t *testing.T){
	t.Parallel()
	helmChartPath, err := filepath.Abs(PegaHelmChartPath)
	require.NoError(t, err)
