...
examples: dependencies
	...
	helm template ./charts/pega/ --output-dir ./build/azure-aks --values ./charts/pega/values.yaml --namespace example --set provider=aks --set actions.execute=deploy
	tar -C ./build/azure-aks/pega/templates -cvzf ./pega-azure-aks-example.tar.gz .

	mkdir -p ./build/google-gke
	helm template ./charts/pega/ --output-dir ./build/google-gke --values ./charts/pega/values.yaml --namespace example --set provider=gke --set actions.execute=deploy
	tar -C ./build/google-gke/pega/templates -cvzf ./pega-google-gke-example.tar.gz .

	mkdir -p ./build/pivotal-pks
	helm template ./charts/pega/ --output-dir ./build/pivotal-pks --values ./charts/pega/values.yaml --namespace example --set provider=pks --set actions.execute=deploy
