...
`platform-services/search-n-reporting-service`  | An image to deploy the required search engine for the latest generation of search and reporting capabilities in Pega Infinity as a backing service. This Docker image contains Pega Search and Reporting Service, refer [Instructions to configure the Pega backingservices](charts/backingservices/README.md) for image tag version details | `<version>` |
...
The docker image `platform-services/search-n-reporting-service` is part of the chart `backingservices/srs`. The `srs` subchart deployment provisions the latest generation of search and reporting capabilities in Pega Infinity; this service is an alternate search feature to the previous `platform/search` image-based search deployment.  [Instructions to configure the Pega backingservices](charts/backingservices/README.md). 
