...
`set_vm_max_map_count`   | Elasticsearch requires `vm.max_map_count` to be configured to an appropriate value. This may be configured manually on your system and if so, you may bypass this privileged init container. For more information, see the [ElasticSearch documentation](https://www.elastic.co/guide/en/elasticsearch/reference/current/vm-max-map-count.html).  Elasticsearch uses an mmapfs directory by default for 64-bit systems to store its indices. Do one of the following:<ul><li>Set the value to `false` and increase the mmap count on your environment by updating the vm.max_map_count setting in /etc/sysctl.conf on your nodes and set the value vm.max_map_count=262144.</li><li>Leave the value set to `true` and Utilize the set-max-map-count initContainer in the example yaml file to automatically configure the setting. If you choose this method, note that the init container runs in privileged mode, so you must verify that your environment allows privileged containers.</li></ul>| `true`
