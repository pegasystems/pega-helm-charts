...
| `trustStore`             | Specify the Truststore certificate file name in `pegasearch.srsMTLS.trustStore`. Make sure both the Infinity & SRS CA Certificates are signed against the Infinity keystore.<br/> Note : When using a trustStore certificate, you must also include a Kubernetes secret name, that contains the trustStore certificate in the global.certificatesSecrets parameter. | `""`               |                                                                                   | `""`               |
| `trustStorePassword`     | Set `pegasearch.srsMTLS.trustStorePassword` to the password for the trustStore certificate.(Use this in case of not using `pegasearch.srsMTLS.external_secret_name`)                                                                                                                                                                                                | `""`               |
...
| `keyStorePassword`       | Set `pegasearch.srsMTLS.keyStorePassword` to the password for the keystore certificate.(Use this in case of not using `pegasearch.srsMTLS.external_secret_name`)                                                                                                                                                                                                    | `""`               |
| `external_secret_name`   | Specify the external secret name containing `SRS_TRUSTSTORE_PASSWORD` and `SRS_KEYSTORE_PASSWORD` for `pegasearch.srsMTLS.external_secret_name`(Ignore this in case of using `pegasearch.srsMTLS.trustStorePassword` & `pegasearch.srsMTLS.keyStorePassword`).                                                                                                      | `""`               | . | `""`               |
Example:Example:
