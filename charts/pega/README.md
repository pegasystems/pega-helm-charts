...
`username` | The plain text username for authentication with the Cassandra cluster.<br/>You will need to change the value in your helm chart to the username supplied by your Cassandra cluster provider. A better practise is to avoid plain text usernames and instead make this parameter blank and include the username in an external secrets manager with the key CASSANDRA_USERNAME. | dnode_ext
`password` | The plain text password for authentication with the Cassandra cluster.<br/>You will need to change the value in your helm chart to the password supplied by your Cassandra cluster provider. A better practice is to avoid plain text passwords and instead make this parameter  blank and include the pasword in an external secrets manager with the key CASSANDRA_PASSWORD. | dnode_ext
...
`trustStorePassword` | If required provide trustStorePassword value in plain text. A better practise is to avoid plain text passwords and instead include the password in an external secrets manager with the key CASSANDRA_TRUSTSTORE_PASSWORD. | Empty
...
`keyStorePassword` | If required provide keyStorePassword value in plain text. A better practise is to avoid plain text passwords and instead include the password in an external secrets manager with the key CASSANDRA_KEYSTORE_PASSWORD. | Empty
