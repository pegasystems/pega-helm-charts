...
# Stream (externalized Kafka service) settings.
stream:
  # Enabled by default. When enabled, the deployment uses external Kafka as configured below and does not use the "Stream" node type at the tier level. Disable this setting to use the embedded "Stream" node type.
  enabled: true
