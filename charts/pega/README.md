...
---       | ---                               | ---
`port`    | The port of the tier to be exposed to the cluster. For HTTP this is generally `80`. To disable the port 80 remove this and make sure you enable the service `tls`. | `80`
`targetPort`    | The target port of the container to expose. The Pega container exposes web traffic on port `8080`. Remove this target port if you are disabling the port `80`. | `8080`
