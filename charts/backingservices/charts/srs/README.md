...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>Enabled</td>
            <td>Elasticsearch 7.17.9, 7.17.29, 8.10.3, 8.15.1, 8.15.5, 8.18.2, 8.18.3, 8.18.6, 8.19.11 & 8.19.18</td>
            <td>As a best practice, use Elasticsearch version 8.19.18.</td>
        </tr>
        <tr>
...
To migrate to Elasticsearch version 7.17.9, 7.17.29, 8.10.3, 8.15.1, 8.15.5, 8.18.2, 8.18.3, 8.18.6, 8.19.11 or 8.19.18 from the Elasticsearch version 7.10.2 or 7.16.3, perform the following steps:
...
3. To update Elasticsearch version to 8.10.3, 8.15.1, 8.15.5, 8.18.2, 8.18.3, 8.18.6, 8.19.11 or 8.19.18, perform the following actions:
   * Update the Elasticsearch `dependencies.version` parameter in the [requirement.yaml](../../requirements.yaml) to 8.5.1.

     Note: This parameter references the Elasticsearch Helm chart version and not the Elasticsearch cluster version.
   * Update the elasticsearch.imageTag in the Backing Services Helm chart to 8.10.3, 8.15.1, 8.15.5, 8.18.2, 8.18.3, 8.18.6, 8.19.11 or 8.19.18.
...
3. To use Elasticsearch version 8.10.3, 8.15.1, 8.15.5, 8.18.2, 8.18.3, 8.18.6, 8.19.11 or 8.19.18, upgrade your external Elasticsearch cluster to 8.10.3, 8.15.1, 8.15.5, 8.18.2, 8.18.3, 8.18.6, 8.19.11 or 8.19.18 according to your organization’s best practices. For more information, see official Elasticsearch version 8.x documentation.
4. Restart the SRS pods.
...
| `srsStorage.provisionInternalESCluster`  | <b>Note: You cannot use OpenSearch for the internally provisioned cluster.</b><br><br><ol><li>Set the `srsStorage.provisionInternalESCluster` parameter to `true` to provide an internally managed and secured Elasticsearch cluster.</li><li>In the [requirements.yaml](../../requirements.yaml) file, set the `dependencies.version` parameter to the same version you configured for the `elasticsearch.imageTag` version in the Backing Services Helm chart [values.yaml](../../values.yaml) file.</li><li>From the Backing Services Helm chart directory in your environment, run the following command to create your Elasticsearch certificates and pass them to secrets: <p>`$ make es-prerequisite NAMESPACE=<NAMESPACE_USED_FOR_DEPLOYMENT> ELASTICSEARCH_VERSION=<ELASTICSEARCH_VERSION>`</p><p>Where `NAMESPACE` references your deployment namespace of the SRS cluster and `ELASTICSEARCH_VERSION` matches the Elasticsearch version you want to use in [values.yaml](../../values.yaml) and [requirements.yaml](../../requirements.yaml).</p><p>To use a supported official Elasticsearch image copied to an internal registry, override the complete image reference, including the tag:</p><p>`$ make es-prerequisite NAMESPACE=<NAMESPACE_USED_FOR_DEPLOYMENT> ELASTICSEARCH_IMAGE=<REGISTRY>/<REPOSITORY>:<TAG>`</p><p>The image tag must match the Elasticsearch version configured in [values.yaml](../../values.yaml) and [requirements.yaml](../../requirements.yaml).</p><p>For a private registry, set the registry password as a masked automation environment secret, not as a command-line argument. The Makefile passes it to Docker through <code>--password-stdin</code> and uses a temporary Docker configuration:</p><p><code>export ELASTICSEARCH_REGISTRY_PASSWORD="$REGISTRY_PASSWORD_SECRET"</code><br><code>make es-prerequisite NAMESPACE=&lt;NAMESPACE_USED_FOR_DEPLOYMENT&gt; ELASTICSEARCH_IMAGE=&lt;REGISTRY&gt;/&lt;REPOSITORY&gt;:&lt;TAG&gt; ELASTICSEARCH_REGISTRY=&lt;REGISTRY_HOST&gt; ELASTICSEARCH_REGISTRY_USERNAME=&lt;REGISTRY_USERNAME&gt;</code></p><p>If `ELASTICSEARCH_REGISTRY` is omitted, the first host component of `ELASTICSEARCH_IMAGE` is used as the Docker login server. The image tag must match the Elasticsearch version configured in [values.yaml](../../values.yaml) and [requirements.yaml](../../requirements.yaml).</p></li></ol> |
