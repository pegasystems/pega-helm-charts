...
| \>= 8.6               | \>=1.17.20  | 7.10.2 or 7.16.3      | Pega Infinity 8.6 and later supports using a Pega-provided platform-services/search-n-reporting-service Docker Image. While all SRS Docker image versions starting at 1.12.0 are certified against Elasticsearch versions 7.10.2 and 7.16.3, Pega recommends using the latest available SRS image - 1.17.20 or later. |
...
To connect to an external Elasticsearch service, add the following settings.
You must place the required certificates to authenticate with an external Elasticsearch service in an accessible location and appropriately run the make command. 
Pega only supports .p12 and .jks certificates for Elasticsearch authentication.
eg: If certs are placed under /home/certs. Make command will look like this:
For example, if you place the certificates under /home/certs, run the command: make external-es-secrets NAMESPACE=pegabackingservices ELASTICSEARCH_VERSION=7.10.2 PATH_TO_CERTIFICATE=/home/certs/truststore.jks.
