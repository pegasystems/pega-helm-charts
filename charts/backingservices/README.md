...
\>= 8.6                 | \>=1.12.0 \<=1.16.1     | 7.9.3                    | Pega Infinity 8.6 and later supports using a Pega-provided platform-services/search-n-reporting-service Docker Image that is tagged with version 1.12.0 and later(recommended 1.16.1 (latest)). Current SRS versions are certified to support Elasticsearch version 7.9.3.
...
`srsStorage.provisionInternalESCluster` | <ul><li>Enable this parameter to provision an internally managed and secured Elasticsearch cluster to be used with SRS (Make sure you please download `Makefile` from the [repository](https://github.com/pegasystems/pega-helm-charts) ( path from root : `pega-helm-charts/charts/backingservices/Makefile`) and replace `<YOUR_NAMESPACE> ` with `namespace` used for the deployment run `$ make  ` or `$ make es-prerequisite`).</li><li>Disable this parameter to use your own Elasticsearch service with SRS. Use the uncommented section to specify the connection details for your existing external ElasticSearch cluster that is available to your deployment. For Elasticsearch clusters secured with Basic Authentication, use the commented section 'srsStorage.basicAuthentication' to provide access credentials. For AWS Elasticsearch clusters secured with IAM role based authentication, use 'srsStorage.awsIAM' to set the aws region where AWS Elasticsearch cluster is hosted.</li></ul>
