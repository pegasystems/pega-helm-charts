...
tar -czvf ${DEPLOY_CONFIGURATIONS_FILE_NAME} --directory=./charts/pega/config deploy/context.xml.tmpl deploy/server.xml.tmpl deploy/prconfig.xml deploy/prlog4j2.xml.tmpl deploy/java.security.overwrite.tmpl deploy/catalina.properties deploy/tomcat-web.xml
