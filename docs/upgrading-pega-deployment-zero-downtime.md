...
2. Use a text editor to open the pega.yaml file and update the following parameters in the chart based on your Kubernetes environment requirements:
   ...
   | installer.upgrade.pegaRESTUsername | For zero-downtime upgrades, specify username to access Pega REST server within the quotes that the process uses to perform pre-upgrade/post-upgrade actions required for the upgrade orchestration. (Will be ignore if installer.upgrade.pega_rest_external_secret_name value is specified.)                                                                                                                   | pegaRESTUsername: "" |
   | installer.upgrade.pegaRESTPassword | For zero-downtime upgrades, specify user's password to access Pega REST server within the quotes that the process uses to perform pre-upgrade/post-upgrade actions required for the upgrade orchestration. (Will be ignore if installer.upgrade.pega_rest_external_secret_name value is specified.)                                                                                                                                                                                                     | pegaRESTPassword: "" |
   | installer.upgrade.pega_rest_external_secret_name | For zero-downtime upgrades, specify an external secret name to access Pega REST server within quotes the the process uses to perform pre-upgrade/post-upgrade actions required for the upgrade orchestration.                                                                                                                                                                                                  
