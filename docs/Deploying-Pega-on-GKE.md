...
2. Use a text editor to open the pega.yaml file and update the following parameters in the chart based on your GKE requirements:
    ...
    | docker.imagePullSecretNames | Specify the docker registry secrets to pull an image from private repo. Refer to [Kubernetes Secrets](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/) to create docker registry secrets. |
