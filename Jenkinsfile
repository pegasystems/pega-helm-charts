...
node {
      ...
      stage ("Checkout and Package Charts and ConfigFiles") {
            ...
            sh "tar -czvf ${deployConfigsFileName} --directory=./charts/pega/config deploy/context.xml.tmpl deploy/server.xml.tmpl deploy/prconfig.xml deploy/prlog4j2.xml deploy/java.security.overwrite"
